...
const listFoods = (recipe) => {
  ...
  names.forEach(value =>{
    result.push(value.slice(value.indexOf(' ')+1).slice(value.slice(value.indexOf(' ')+1).indexOf(' ')+1, value.slice(value.indexOf(' ')+1).length));
  });
  return result;
};
...
const splitFoods = (recipe) => {
  ...
  let names = recipe.ingredients;
  names.forEach(value =>{
    result.push(value.split(' ').slice(2,value.length).join(' '));
  });
  return result;
};
...
const stepActions = (recipe) => {
  let result = [];
  // Solution code here...
  let names = recipe.steps;
  names.forEach(value =>{
    result.push(value.split(' ').shift());
  });
  return result;
};
