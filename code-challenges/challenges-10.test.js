...
const getNames = (arr) => {
  // Solution code here...
  let rev = arr.map(value => value.name).map(value=>{
    return value.split('').reduce((acc, val, index)=>{
      acc+=value.split('')[value.split('').length-1 -index];
      return acc;
    },'');
  });
  return rev;
...
const count = (target, input) => {
  ...
  let counter =0;
  input.map(row =>{
    row.map(value => value===target?counter+=1:counter);
  });
  return counter;
...
let findMaleAndFemale = (data) => {
  ...
  }).map(val => data.indexOf(val) < data.length-1?val.name+ ' and':val.name).join(' ');
...
let findShortest = (data) => {
  // Solution code here...
  let minChar = data[0].name;
  let arr = data.map(value => value.name).filter(val =>{
    if(val.length <= minChar.length){
      minChar = val;
      return val;
    }
  });
